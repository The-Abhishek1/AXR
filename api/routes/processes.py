...
# ✅ SPECIFIC ROUTES FIRST - these have static paths

@router.get("/processes/steps/{step_id}/retry")
...
@router.get("/processes/steps/{step_id}")
...
def list_processes(request: Request):
    ...
    return {"count": len(response), "processes": response}

# ✅ PARAMETERIZED ROUTES LAST - these have dynamic paths
...
def get_process_detail(pid: str, request: Request):
    ...
    return {
        ...
        "current_step": str(process.current_step_id) if process.current_step_id else None,
        "steps": step_details,
    }