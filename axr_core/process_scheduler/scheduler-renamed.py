...
class ProcessScheduler:
        ...
        for process in active_processes:
            ...
            for step in runnable_steps:
                ...
                if not self.security_evaluator.allow(process, step):
                    ...
                    process.fail("Security policy violation")
                    self.repo.save_process(process)

                    self.event_bus.publish(
                        Event(event_type="PROCESS_FAILED", pid=process.pid)
                    )

                    # 🔴 CRITICAL: stop scheduling further steps
                    return
                ...
                print(f"[FAIR-DEBUG] pid={process.pid} active="
    ...
    def _execute_step(self, process: AIProcess, step: ProcessStep) -> None:
        ...
        try:
            ...
            self.checkpoint_manager.save_checkpoint(process, self.steps[process.pid])
            ...
            if not workers:
                ...
                time.sleep(0.2)  # small backoff to prevent hot loop
                return
            ...
            # routing / capacity conditions should not rollback process
            if isinstance(e, RuntimeError) and "No workers" in str(e):
                return
    ...
    def cancel_process(self, pid: UUID):
        ...
        # cancel running steps + clear leases
        for step in steps:
            if step.status == StepStatus.RUNNING:
                step.fail("Cancelled")

            # remove lease if exists
